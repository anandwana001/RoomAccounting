...
typealias ViewHolderFactory<T> = (ViewGroup) -> AtlysRecyclerViewViewHolder<T>

/**
 * Generic RecyclerView for AtlysAccounting App.
 */
class AtlysRecyclerViewAdapter<T : Any> internal constructor(
...
) : RecyclerView.Adapter<AtlysRecyclerViewViewHolder<T>>() {
  ...
  override fun onBindViewHolder(holder: AtlysRecyclerViewViewHolder<T>, position: Int) =
  ...
  private fun setData(newDataList: List<T>) {
    val result = DiffUtil.calculateDiff(
      AtlysDiffCallback(dataList, newDataList),
      /* detectMoves= */ false
    )
    dataList.clear()
